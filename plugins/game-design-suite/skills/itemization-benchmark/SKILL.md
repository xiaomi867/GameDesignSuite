---
name: itemization-benchmark
description: 负责把公开商业游戏的装备、武器、遗器、圣遗物、声骸等 Itemization 数据整理成可比较的参考模型，用于设计启发、数值曲线对标、结构验证和风险诊断。首批参考覆盖崩坏：星穹铁道、原神、鸣潮；不把外部游戏数值直接当成本项目标准，也不替代 itemization-design 对本项目做最终设计决策。
---
...
- [Reference Extraction & Normalization](references/reference-extraction-and-normalization.md)
- [HSR Relic Patterns](references/hsr-relic-patterns.md)
- [Genshin Weapon & Artifact Patterns](references/genshin-weapon-artifact-patterns.md)
- [Wuthering Waves Weapon & Echo Patterns](references/wuwa-weapon-echo-patterns.md)
- [Cross-Game Itemization Benchmark](references/cross-game-itemization-benchmark.md)
