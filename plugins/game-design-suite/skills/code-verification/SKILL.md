...
description: 只读验证游戏客户端/服务器代码如何读取和执行设计与配置，包括字段解析、默认值、枚举、Target、Buff、状态、存档、网络、结算和上传校验。用于确认“表里这样填到底是否生效”，不负责大规模重构实现。无论用户是否提醒，每个独立正式结果前都必须显示【本次专业视角】并标出主责/协同。
---
